...
import server.DataSourceClasses.NotificationDataSource;
...
public class MockDatabase {
    ...
    public static Response queryNotifications(Request request) throws InvalidArgumentValueException {
        DataCollection<Notification> returnNotifications = new DataCollection<>();
        for(Object[] row : notifications){
            Notification notification = new Notification().setNotificationId((int) row[0]).setMessage((String) row[3]);
            //get an array of string ID
            String receiversIdString = (String) row[1];
            String readersIdString  = (String) row[2];
            String[] receiverIds =  receiversIdString.split(",");
            String[] readerIds =  readersIdString.split(",");
            // Append receiver IDs
        ...
        return new Response(true, returnNotifications);
    }
    ...
    /**
     * Find a notification in the database, identified by ID.
     *
     * @param notification The notification to find
     * @return The index of the notification in the database.
     */
    private static int find(Notification notification) {
        int key = notification.getNotificationId();
        Object[] match = new Object[]{};
        for (Object[] row : users) {
            if (row[0].equals(key)) {
                match = row;
        ...
        return notifications.indexOf(match);
    }
     ...
     * @param notificationList The overriding notification
     * @return The response containing the state if the request was fulfilled, as well as an attachment (after change)
     */
    public static Response edit(DataCollection<Notification> notificationList, User sender){
        for (Notification notification : notificationList){
            int index = find(notification);
            Object[] overrideRow = new Object[] {notification.getNotificationId(), notification.getReceivers(), notification.getReaders(), notification.getMessage()};
            notifications.set(index, overrideRow);
        }
        DataCollection<Notification> returningNotifications = (DataCollection<Notification>) getFromUnitId(sender.getUnitId()).getAttachment();
     ...
     * receiving list
     */
    public static Response getFromUnitId(Integer unitId){
        DataCollection<Notification> allNotifications = null;
        try {
            allNotifications = (DataCollection<Notification>) queryNotifications(null).getAttachment();
        ...
        return new Response(true, filteredNotifications);
    }
    ...
    public static <T extends IData> Response add(Request<T> request) throws InvalidArgumentValueException {
        ...
        if (type.equals(Request.ObjectType.USER)) {
            ...
            return add((Notification) attachment);
        }
    ...
    private static Response add(Notification attachment) {
        Object[] newRow = new Object[]{notifications.size(), attachment.getReceivers(), attachment.getReaders(), attachment.getMessage()};
        Response response = new Response(true, null);
        notifications.add(newRow);
        return response;
    }
     ...
     */
    private static void pushNotification(Order order) throws InvalidArgumentValueException {
        // Link the order with an asset
        Object[] linkedAsset = assets.get(order.getAssetId());
     ...
     * @param order
     */
    private static Order matchOrder(Order order) throws InvalidArgumentValueException {
