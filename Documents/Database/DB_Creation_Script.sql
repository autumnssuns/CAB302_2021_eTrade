...
-- -----------------------------------------------------
-- Table `cab302_eTrade`.`sell_orders`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cab302_eTrade`.`sell_orders` (
...
CREATE INDEX `organisation_idx` ON `cab302_eTrade`.`sell_orders` (`organisation_id` ASC, `asset_id` ASC) VISIBLE;


-- -----------------------------------------------------
-- Table `cab302_eTrade`.`buy_orders`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cab302_eTrade`.`buy_orders` (
  `order_id` INT NOT NULL AUTO_INCREMENT,
  `organisation_id` INT NOT NULL,
  `asset_id` INT NOT NULL,
  `placed_quantity` INT NOT NULL DEFAULT 0,
  `bought_quantity` INT NOT NULL DEFAULT 0,
  `price` DECIMAL(2) NOT NULL,
...
CREATE INDEX `organisation_idx` ON `cab302_eTrade`.`buy_orders` (`organisation_id` ASC, `asset_id` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `cab302_eTrade`.`transactions` (
  ...
  CONSTRAINT `sell`
    ...
    REFERENCES `cab302_eTrade`.`sell_orders` (`order_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `buy`
    FOREIGN KEY (`buy_order_id`)
    REFERENCES `cab302_eTrade`.`buy_orders` (`order_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `buy_idx` ON `cab302_eTrade`.`transactions` (`buy_order_id` ASC) INVISIBLE;
